...
public class ESICharacterKillmaillSyncTest extends SyncTestBase {
  ...
  public void setup() throws Exception {
    ...
    ESIEndpointSyncTracker.getOrCreateUnfinishedTracker(charSyncAccount, ESISyncEndpoint.CHAR_KILL_MAIL, 1234L);
  ...
  private void setupOkMock() throws Exception {
    ...
    // Setup kill list
    List<GetCharactersCharacterIdKillmailsRecent200Ok> killmailList =
        Arrays.stream(killmailTestData).map(x -> {
          GetCharactersCharacterIdKillmailsRecent200Ok killHash = new GetCharactersCharacterIdKillmailsRecent200Ok();
  ...
  private void verifyDataUpdate(Object[][] testData) throws Exception {
    ...
    List<Kill> storedKills = AbstractESIAccountSync.retrieveAll(testTime, (long contid, AttributeSelector at) ->
        Kill.accessQuery(charSyncAccount, contid, 1000, false, at, AbstractESIAccountSync.ANY_SELECTOR,
    ...
    List<KillVictim> storedVictims = AbstractESIAccountSync.retrieveAll(testTime, (long contid, AttributeSelector at) ->
        KillVictim.accessQuery(charSyncAccount, contid, 1000, false, at, AbstractESIAccountSync.ANY_SELECTOR,
                               AbstractESIAccountSync.ANY_SELECTOR, AbstractESIAccountSync.ANY_SELECTOR,
                               AbstractESIAccountSync.ANY_SELECTOR, AbstractESIAccountSync.ANY_SELECTOR,
                               AbstractESIAccountSync.ANY_SELECTOR, AbstractESIAccountSync.ANY_SELECTOR,
                               AbstractESIAccountSync.ANY_SELECTOR, AbstractESIAccountSync.ANY_SELECTOR,
    ...
    List<KillAttacker> storedAttackers = AbstractESIAccountSync.retrieveAll(testTime, (long contid, AttributeSelector at) ->
        KillAttacker.accessQuery(charSyncAccount, contid, 1000, false, at, AbstractESIAccountSync.ANY_SELECTOR,
                               AbstractESIAccountSync.ANY_SELECTOR, AbstractESIAccountSync.ANY_SELECTOR,
                               AbstractESIAccountSync.ANY_SELECTOR, AbstractESIAccountSync.ANY_SELECTOR,
                               AbstractESIAccountSync.ANY_SELECTOR, AbstractESIAccountSync.ANY_SELECTOR,
                               AbstractESIAccountSync.ANY_SELECTOR, AbstractESIAccountSync.ANY_SELECTOR,
    ...
    List<KillItem> storedItems = AbstractESIAccountSync.retrieveAll(testTime, (long contid, AttributeSelector at) ->
        KillItem.accessQuery(charSyncAccount, contid, 1000, false, at, AbstractESIAccountSync.ANY_SELECTOR,
                                 AbstractESIAccountSync.ANY_SELECTOR, AbstractESIAccountSync.ANY_SELECTOR,
                                 AbstractESIAccountSync.ANY_SELECTOR, AbstractESIAccountSync.ANY_SELECTOR,
                                 AbstractESIAccountSync.ANY_SELECTOR, AbstractESIAccountSync.ANY_SELECTOR,
                                 AbstractESIAccountSync.ANY_SELECTOR));
  ...
  public void testSyncUpdate() throws Exception {
    ...
    // Perform the sync
    ESICharacterKillMailSync sync = new ESICharacterKillMailSync(charSyncAccount);
    ...
    ESIEndpointSyncTracker syncTracker = ESIEndpointSyncTracker.getLatestFinishedTracker(charSyncAccount,
                                                                                         ESISyncEndpoint.CHAR_KILL_MAIL);
  ...
  public void testSyncUpdateExisting() throws Exception {
    ...
    for (int i = 0; i < allData.length / 2; i++) {
      ...
      Kill nk = new Kill((Integer) dt[0],
                         ...
                         (Integer) dt[3],
                         (Integer) dt[4]);
      nk.setup(charSyncAccount, testTime - 1);
      ...
      KillVictim nv = new KillVictim((Integer) victim[0],
                                     ...
                                     (Double) victim[8],
                                     (Double) victim[9]);
      nv.setup(charSyncAccount, testTime - 1);
      ...
      for (Object[] nextAttacker : attackers) {
        ...
        na.setup(charSyncAccount, testTime - 1);
