...
public class CharacterStandingSyncTest extends SyncTestBase {
  ...
  @Test
  public void testCharacterStandingSyncUpdate() throws Exception {
    setupOkMock();
    EasyMock.replay(mockServer);
    long testTime = 1234L;

    // Perform the sync
    SyncStatus syncOutcome = CharacterStandingSync.syncStanding(testTime, syncAccount, syncUtil, mockServer);
    ...
    Assert.assertEquals(testDate, Capsuleer.getCapsuleer(syncAccount).getStandingsExpiry());
    Assert.assertEquals(SyncState.UPDATED, CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getStandingsStatus());
    Assert.assertNull(CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getStandingsDetail());
  }

  // Test update with standings already populated
  @Test
  public void testCharacterStandingSyncUpdateExisting() throws Exception {
    // Prepare mock
    setupOkMock();
    EasyMock.replay(mockServer);
    long testTime = 1234L;
  ...
  @Test
  public void testCharacterStandingSyncUpdateSkip() throws Exception {
    // Prepare mock
    setupOkMock();
    EasyMock.replay(mockServer);
    long testTime = 1234L;
    ...
    Assert.assertEquals(prevDate, Capsuleer.getCapsuleer(syncAccount).getStandingsExpiry());
    Assert.assertEquals(SyncState.UPDATED, CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getStandingsStatus());
    Assert.assertNull(CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getStandingsDetail());
  }
