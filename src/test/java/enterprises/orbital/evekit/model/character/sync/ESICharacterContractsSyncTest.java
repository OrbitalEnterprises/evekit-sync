...
public class ESICharacterContractsSyncTest extends SyncTestBase {
  ...
  static {
    ...
    int typeLen = GetCharactersCharacterIdContracts200Ok.TypeEnum.values().length;
    int statusLen = GetCharactersCharacterIdContracts200Ok.StatusEnum.values().length;
    int availLen = GetCharactersCharacterIdContracts200Ok.AvailabilityEnum.values().length;
    for (int i = 0; i < size; i++) {
      ...
      contractsTestData[i][7] = GetCharactersCharacterIdContracts200Ok.TypeEnum.values()[TestBase.getRandomInt(typeLen)];
      contractsTestData[i][8] = GetCharactersCharacterIdContracts200Ok.StatusEnum.values()[TestBase.getRandomInt(statusLen)];
      contractsTestData[i][9] = TestBase.getRandomText(50);
      contractsTestData[i][10] = TestBase.getRandomBoolean();
      contractsTestData[i][11] = GetCharactersCharacterIdContracts200Ok.AvailabilityEnum.values()[TestBase.getRandomInt(availLen)];
    ...
    Object[] auctionContracts = Arrays.stream(contractsTestData)
                                      .filter(x -> x[7] == GetCharactersCharacterIdContracts200Ok.TypeEnum.AUCTION)
                                      .toArray();
  ...
  public void setup() throws Exception {
    ...
    ESIEndpointSyncTracker.getOrCreateUnfinishedTracker(charSyncAccount, ESISyncEndpoint.CHAR_CONTRACTS, 1234L, null);
  ...
  private void setupOkMock() throws Exception {
    ...
    List<GetCharactersCharacterIdContracts200Ok> contractList =
        Arrays.stream(contractsTestData)
              .map(x -> {
                GetCharactersCharacterIdContracts200Ok nextContract = new GetCharactersCharacterIdContracts200Ok();
                ...
                nextContract.setType((GetCharactersCharacterIdContracts200Ok.TypeEnum) x[7]);
                nextContract.setStatus((GetCharactersCharacterIdContracts200Ok.StatusEnum) x[8]);
                nextContract.setTitle((String) x[9]);
                nextContract.setForCorporation((Boolean) x[10]);
                nextContract.setAvailability((GetCharactersCharacterIdContracts200Ok.AvailabilityEnum) x[11]);
    ...
    for (int i = 0; i < pages.length; i++) {
      ...
      ApiResponse<List<GetCharactersCharacterIdContracts200Ok>> apir = new ApiResponse<>(200, headers,
                                                                                      contractList.subList(last,
                                                                                                        pages[i]));
      EasyMock.expect(mockEndpoint.getCharactersCharacterIdContractsWithHttpInfo(
    ...
    for (int contractID : contractsWithItems) {
      List<GetCharactersCharacterIdContractsContractIdItems200Ok> itemList =
          Arrays.stream(itemsTestData).filter(x -> (Integer) x[0] == contractID)
          .map(x -> {
            GetCharactersCharacterIdContractsContractIdItems200Ok nextItem = new GetCharactersCharacterIdContractsContractIdItems200Ok();
      ...
      Map<String, List<String>> headers = createHeaders("Expires", "Thu, 21 Dec 2017 12:00:00 GMT", "X-Pages",
                                                        String.valueOf(pages.length));
      ApiResponse<List<GetCharactersCharacterIdContractsContractIdItems200Ok>> apir = new ApiResponse<>(200, headers, itemList);
      EasyMock.expect(mockEndpoint.getCharactersCharacterIdContractsContractIdItemsWithHttpInfo(
          EasyMock.eq((int) charSyncAccount.getEveCharacterID()),
    ...
    for (int contractID : contractsWithBids) {
      List<GetCharactersCharacterIdContractsContractIdBids200Ok> bidList =
          Arrays.stream(bidsTestData).filter(x -> (Integer) x[1] == contractID)
                .map(x -> {
                  GetCharactersCharacterIdContractsContractIdBids200Ok nextBid = new GetCharactersCharacterIdContractsContractIdBids200Ok();
      ...
      Map<String, List<String>> headers = createHeaders("Expires", "Thu, 21 Dec 2017 12:00:00 GMT", "X-Pages",
                                                        String.valueOf(pages.length));
      ApiResponse<List<GetCharactersCharacterIdContractsContractIdBids200Ok>> apir = new ApiResponse<>(200, headers, bidList);
      EasyMock.expect(mockEndpoint.getCharactersCharacterIdContractsContractIdBidsWithHttpInfo(
          EasyMock.eq((int) charSyncAccount.getEveCharacterID()),
          EasyMock.eq(contractID),
          EasyMock.isNull(),
          EasyMock.anyString(),
  ...
  private void verifyDataUpdate() throws Exception {
    ...
    List<Contract> storedData = AbstractESIAccountSync.retrieveAll(testTime, (long contid, AttributeSelector at) ->
        Contract.accessQuery(charSyncAccount, contid, 1000, false, at, AbstractESIAccountSync.ANY_SELECTOR,
    ...
    List<ContractItem> storedItemData = AbstractESIAccountSync.retrieveAll(testTime, (long contid, AttributeSelector at) ->
        ContractItem.accessQuery(charSyncAccount, contid, 1000, false, at, AbstractESIAccountSync.ANY_SELECTOR,
                                 AbstractESIAccountSync.ANY_SELECTOR, AbstractESIAccountSync.ANY_SELECTOR,
                                 AbstractESIAccountSync.ANY_SELECTOR, AbstractESIAccountSync.ANY_SELECTOR,
                                 AbstractESIAccountSync.ANY_SELECTOR, AbstractESIAccountSync.ANY_SELECTOR));

    List<ContractBid> storedBidData = AbstractESIAccountSync.retrieveAll(testTime, (long contid, AttributeSelector at) ->
        ContractBid.accessQuery(charSyncAccount, contid, 1000, false, at, AbstractESIAccountSync.ANY_SELECTOR,
                                 AbstractESIAccountSync.ANY_SELECTOR, AbstractESIAccountSync.ANY_SELECTOR,
                                 AbstractESIAccountSync.ANY_SELECTOR, AbstractESIAccountSync.ANY_SELECTOR));
  ...
  public void testSyncUpdate() throws Exception {
    ...
    // Perform the sync
    ESICharacterContractsSync sync = new ESICharacterContractsSync(charSyncAccount);
    ...
    ESIEndpointSyncTracker syncTracker = ESIEndpointSyncTracker.getLatestFinishedTracker(charSyncAccount,
                                                                                         ESISyncEndpoint.CHAR_CONTRACTS);
    ...
    syncTracker = ESIEndpointSyncTracker.getUnfinishedTracker(charSyncAccount, ESISyncEndpoint.CHAR_CONTRACTS);
