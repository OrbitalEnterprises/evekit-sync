...
public class CharacterFacWarStatsSyncTest extends SyncTestBase {
  ...
  @Test
  public void testCharacterFacWarStatsSyncUpdate() throws Exception {
    setupOkMock();
    EasyMock.replay(mockServer);
    long testTime = 1234L;

    // Perform the sync
    SyncStatus syncOutcome = CharacterFacWarStatsSync.syncFacWarStats(testTime, syncAccount, syncUtil, mockServer);
    ...
    Assert.assertEquals(testDate, Capsuleer.getCapsuleer(syncAccount).getFacWarStatsExpiry());
    Assert.assertEquals(SyncState.UPDATED, CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getFacWarStatsStatus());
    Assert.assertNull(CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getFacWarStatsDetail());
  }

  // Test update with stats already populated
  @Test
  public void testCharacterFacWarStatsSyncUpdateExisting() throws Exception {
    // Prepare mock
    setupOkMock();
    EasyMock.replay(mockServer);
    long testTime = 1234L;
  ...
  @Test
  public void testCharacterFacWarStatsSyncUpdateSkip() throws Exception {
    // Prepare mock
    setupOkMock();
    EasyMock.replay(mockServer);
    long testTime = 1234L;
    ...
    Assert.assertEquals(prevDate, Capsuleer.getCapsuleer(syncAccount).getFacWarStatsExpiry());
    Assert.assertEquals(SyncState.UPDATED, CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getFacWarStatsStatus());
    Assert.assertNull(CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getFacWarStatsDetail());
  }
