...
public class CharacterContactListSyncTest extends SyncTestBase {
  ...
  static {
    ...
    for (int i = 0; i < size; i++) {
      testData[i][0] = listTypes[TestBase.getRandomInt(3)];
      ...
      testData[i][7] = listTypes[TestBase.getRandomInt(3)];
  ...
  public void setupOkMock() throws Exception {
    ...
    IContactSet contactSet = new IContactSet() {
      ...
      public List<IContact> getContacts() {
        return collectIContact("character", testData);
      }
      ...
      public List<IContactLabel> getContactLabels() {
        return collectIContactLabel("character", testData);
      }
  ...
  public void testContactListSyncUpdate() throws Exception {
    ...
    // Perform the sync
    SyncStatus syncOutcome = CharacterContactListSync.syncContactList(testTime, syncAccount, syncUtil, mockServer);
    ...
    Assert.assertEquals(testDate, Capsuleer.getCapsuleer(syncAccount).getContactListExpiry());
    Assert.assertEquals(SyncState.UPDATED, CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getContactListStatus());
    Assert.assertNull(CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getContactListDetail());
  }
  ...
  public void testContactListSyncUpdateSkip() throws Exception {
    ...
    Assert.assertEquals(prevDate, Capsuleer.getCapsuleer(syncAccount).getContactListExpiry());
    Assert.assertEquals(SyncState.UPDATED, CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getContactListStatus());
    Assert.assertNull(CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getContactListDetail());
  }
  ...
  public void testContactListSyncUpdateDelete() throws Exception {
    ...
    for (int i = 0; i < 5; i++) {
      String list = listTypes[TestBase.getRandomInt(3)];
