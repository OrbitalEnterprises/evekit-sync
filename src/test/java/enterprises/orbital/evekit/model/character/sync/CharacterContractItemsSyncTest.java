...
public class CharacterContractItemsSyncTest extends SyncTestBase {
  ...
  public void testContractItemsSyncUpdate() throws Exception {
    ...
    // Perform the sync
    SyncStatus syncOutcome = CharacterContractItemsSync.syncCharacterContractItems(testTime, syncAccount, syncUtil, mockServer);
    ...
    Assert.assertEquals(testDate, Capsuleer.getCapsuleer(syncAccount).getContractItemsExpiry());
    Assert.assertEquals(SyncState.UPDATED, CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getContractItemsStatus());
    Assert.assertNull(CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getContractItemsDetail());
  }
  ...
  public void testContractSyncUpdateSkip() throws Exception {
    ...
    Assert.assertEquals(prevDate, Capsuleer.getCapsuleer(syncAccount).getContractItemsExpiry());
    Assert.assertEquals(SyncState.UPDATED, CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getContractItemsStatus());
    Assert.assertNull(CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getContractItemsDetail());
  }
