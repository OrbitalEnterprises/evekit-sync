...
public class CharacterMarketOrderSyncTest extends SyncTestBase {
  ...
  @Test
  public void testCharacterMarketOrderSyncUpdate() throws Exception {
    setupOkMock();
    EasyMock.replay(mockServer);
    long testTime = 1234L;

    // Perform the sync
    SyncStatus syncOutcome = CharacterMarketOrderSync.syncCharacterMarketOrders(testTime, syncAccount, syncUtil, mockServer);
    ...
    Assert.assertEquals(testDate, Capsuleer.getCapsuleer(syncAccount).getMarketOrdersExpiry());
    Assert.assertEquals(SyncState.UPDATED, CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getMarketOrdersStatus());
    Assert.assertNull(CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getMarketOrdersDetail());
  }

  // Test update with stats already populated
  @Test
  public void testCharacterMarketOrderSyncUpdateExisting() throws Exception {
    // Prepare mock
    setupOkMock();
    EasyMock.replay(mockServer);
    long testTime = 1234L;
  ...
  @Test
  public void testCharacterMarketOrderSyncUpdateSkip() throws Exception {
    // Prepare mock
    setupOkMock();
    EasyMock.replay(mockServer);
    long testTime = 1234L;
    ...
    Assert.assertEquals(prevDate, Capsuleer.getCapsuleer(syncAccount).getMarketOrdersExpiry());
    Assert.assertEquals(SyncState.UPDATED, CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getMarketOrdersStatus());
    Assert.assertNull(CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getMarketOrdersDetail());
  }
  ...
  @Test
  public void testCharacterMarketOrderOpenStateChange() throws Exception {
    long testTime = 1234L;
