...
public class CharacterContractsSyncTest extends SyncTestBase {
    ...
    for (int i = 0; i < testData.length; i++) {
      ...
      contracts.add(new IContract() {
        ...
        public BigDecimal getPrice() {
          return new BigDecimal((Double) instanceData[17]).setScale(2, RoundingMode.HALF_UP);
        }

        @Override
        public BigDecimal getReward() {
          return new BigDecimal((Double) instanceData[18]).setScale(2, RoundingMode.HALF_UP);
        }

        @Override
        public BigDecimal getCollateral() {
          return new BigDecimal((Double) instanceData[19]).setScale(2, RoundingMode.HALF_UP);
        }

        @Override
        public BigDecimal getBuyout() {
          return new BigDecimal((Double) instanceData[20]).setScale(2, RoundingMode.HALF_UP);
        }
  ...
  public void testContractSyncUpdate() throws Exception {
    ...
    Assert.assertEquals(testDate, Capsuleer.getCapsuleer(syncAccount).getContractsExpiry());
    Assert.assertEquals(SyncState.UPDATED, CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getContractsStatus());
    Assert.assertNull(CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getContractsDetail());
  }
  ...
  public void testContractSyncUpdateSkip() throws Exception {
    ...
    Assert.assertEquals(prevDate, Capsuleer.getCapsuleer(syncAccount).getContractsExpiry());
    Assert.assertEquals(SyncState.UPDATED, CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getContractsStatus());
    Assert.assertNull(CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getContractsDetail());
  }
