...
import enterprises.orbital.eve.esi.client.model.PostCharactersCharacterIdAssetsLocationsPosition;
...
public class ESICharacterAssetsSyncTest extends SyncTestBase {
  ...
  static {
    ...
    for (int i = 0; i < size; i++) {
      ...
      assetsTestData[i][2] = GetCharactersCharacterIdAssets200Ok.LocationTypeEnum.values()[TestBase.getRandomInt(
          locationTypeLen)];
      assetsTestData[i][3] = GetCharactersCharacterIdAssets200Ok.LocationFlagEnum.values()[TestBase.getRandomInt(
          locationFlagLen)];
  ...
  public void setup() throws Exception {
    ...
    ESIEndpointSyncTracker.getOrCreateUnfinishedTracker(charSyncAccount, ESISyncEndpoint.CHAR_ASSETS, 1234L, null);
  ...
  private void setupOkMock() throws Exception {
    ...
    List<GetCharactersCharacterIdAssets200Ok> assetList =
        Arrays.stream(assetsTestData)
              .map(x -> {
                GetCharactersCharacterIdAssets200Ok nextAsset = new GetCharactersCharacterIdAssets200Ok();
                nextAsset.setItemId((Long) x[0]);
                nextAsset.setLocationId((Long) x[1]);
                nextAsset.setLocationType((GetCharactersCharacterIdAssets200Ok.LocationTypeEnum) x[2]);
                nextAsset.setLocationFlag((GetCharactersCharacterIdAssets200Ok.LocationFlagEnum) x[3]);
    ...
    for (int i = 0; i < pages.length; i++) {
      ...
      ApiResponse<List<GetCharactersCharacterIdAssets200Ok>> apir = new ApiResponse<>(200, headers,
                                                                                      assetList.subList(last,
                                                                                                        pages[i]));
      EasyMock.expect(mockEndpoint.getCharactersCharacterIdAssetsWithHttpInfo(
    ...
    int BATCH_SIZE = 500;
    List<PostCharactersCharacterIdAssetsLocations200Ok> locationList =
        Arrays.stream(locationsTestData)
              .map(x -> {
                PostCharactersCharacterIdAssetsLocations200Ok nextLocation = new PostCharactersCharacterIdAssetsLocations200Ok();
                nextLocation.setItemId((Long) x[0]);
                PostCharactersCharacterIdAssetsLocationsPosition pos = new PostCharactersCharacterIdAssetsLocationsPosition();
                pos.setX((Double) x[2]);
                pos.setY((Double) x[3]);
                pos.setZ((Double) x[4]);
              ...
              .collect(Collectors.toList());
    List<PostCharactersCharacterIdAssetsNames200Ok> nameList =
        Arrays.stream(locationsTestData)
              .map(x -> {
                PostCharactersCharacterIdAssetsNames200Ok nextName = new PostCharactersCharacterIdAssetsNames200Ok();
    ...
    for (int i = 0; i < locationsTestData.length; i += BATCH_SIZE) {
      ...
      List<PostCharactersCharacterIdAssetsLocations200Ok> locationBatch = locationList.subList(i, last);
      List<PostCharactersCharacterIdAssetsNames200Ok> nameBatch = nameList.subList(i, last);
      List<Long> idList = locationBatch.stream()
                                       .map(PostCharactersCharacterIdAssetsLocations200Ok::getItemId)
                                       .collect(
                                           Collectors.toList());
      Map<String, List<String>> headers = createHeaders("Expires", "Thu, 21 Dec 2017 12:00:00 GMT");
      //noinspection ConstantConditions
      EasyMock.expect(mockEndpoint.postCharactersCharacterIdAssetsLocationsWithHttpInfo(
          EasyMock.eq((int) charSyncAccount.getEveCharacterID()),
          EasyMock.eq(idList),
          EasyMock.isNull(),
          EasyMock.anyString(),
  ...
  private void verifyDataUpdate() throws Exception {
    ...
    List<Asset> storedData = AbstractESIAccountSync.retrieveAll(testTime, (long contid, AttributeSelector at) ->
        Asset.accessQuery(charSyncAccount, contid, 1000, false, at, AbstractESIAccountSync.ANY_SELECTOR,
    ...
    // Check stored data
    Map<Long, Location> locMap = storedData2.stream().collect(Collectors.toMap(Location::getItemID, Function.identity()));
    for (int i = 0; i < assetsTestData.length; i++) {
      long itemID = storedData.get(i).getItemID();
  ...
  public void testSyncUpdate() throws Exception {
    ...
    syncTracker = ESIEndpointSyncTracker.getUnfinishedTracker(charSyncAccount, ESISyncEndpoint.CHAR_ASSETS);
  ...
  public void testSyncUpdateExisting() throws Exception {
    ...
    for (int i = 0; i < assetsTestData.length; i++) {
      ...
      newL.setup(charSyncAccount, testTime - 1);
    ...
    // Check old data
    Map<Long, Location> locMap = oldLs.stream().collect(Collectors.toMap(Location::getItemID, Function.identity()));
    for (int i = 0; i < assetsTestData.length; i++) {
