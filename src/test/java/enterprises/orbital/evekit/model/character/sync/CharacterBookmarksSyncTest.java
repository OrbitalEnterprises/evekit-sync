...
public class CharacterBookmarksSyncTest extends SyncTestBase {
  ...
  public void testBookmarkSyncUpdate() throws Exception {
    ...
    // Perform the sync
    SyncStatus syncOutcome = CharacterBookmarksSync.syncBookmarks(testTime, syncAccount, syncUtil, mockServer);
    ...
    Assert.assertEquals(testDate, Capsuleer.getCapsuleer(syncAccount).getBookmarksExpiry());
    Assert.assertEquals(SyncState.UPDATED, CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getBookmarksStatus());
    Assert.assertNull(CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getBookmarksDetail());
  }
  ...
  public void testBookmarkSyncUpdateSkip() throws Exception {
    ...
    Assert.assertEquals(prevDate, Capsuleer.getCapsuleer(syncAccount).getBookmarksExpiry());
    Assert.assertEquals(SyncState.UPDATED, CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getBookmarksStatus());
    Assert.assertNull(CapsuleerSyncTracker.getUnfinishedTracker(syncAccount).getBookmarksDetail());
  }
