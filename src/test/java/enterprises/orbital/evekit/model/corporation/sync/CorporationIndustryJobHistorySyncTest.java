...
public class CorporationIndustryJobHistorySyncTest extends SyncTestBase {
  ...
  @Test
  public void testCorporationIndustryJobHistorySyncUpdate() throws Exception {
    setupOkMock();
    EasyMock.replay(mockServer);
    long testTime = 1234L;

    // Perform the sync
    SyncStatus syncOutcome = CorporationIndustryJobsHistorySync.syncCorporationIndustryJobsHistory(testTime, syncAccount, syncUtil, mockServer);
    ...
    Assert.assertEquals(testDate, Corporation.getCorporation(syncAccount).getIndustryJobsHistoryExpiry());
    Assert.assertEquals(SyncState.UPDATED, CorporationSyncTracker.getUnfinishedTracker(syncAccount).getIndustryJobsHistoryStatus());
    Assert.assertNull(CorporationSyncTracker.getUnfinishedTracker(syncAccount).getIndustryJobsHistoryDetail());
  }
  ...
  @Test
  public void testCorporationIndustryJobHistorySyncUpdateExisting() throws Exception {
    // Prepare mock
    setupOkMock();
    EasyMock.replay(mockServer);
    long testTime = 1234L;
  ...
  @Test
  public void testCorporationIndustryJobHistorySyncUpdateSkip() throws Exception {
    // Prepare mock
    setupOkMock();
    EasyMock.replay(mockServer);
    long testTime = 1234L;
    ...
    for (int i = 0; i < testData.length; i++) {
      CachedData.update(makeJob(testTime, testData[i], null, null, null, null));
    }
    ...
    Assert.assertEquals(prevDate, Corporation.getCorporation(syncAccount).getIndustryJobsHistoryExpiry());
    Assert.assertEquals(SyncState.UPDATED, CorporationSyncTracker.getUnfinishedTracker(syncAccount).getIndustryJobsHistoryStatus());
    Assert.assertNull(CorporationSyncTracker.getUnfinishedTracker(syncAccount).getIndustryJobsHistoryDetail());
  }
