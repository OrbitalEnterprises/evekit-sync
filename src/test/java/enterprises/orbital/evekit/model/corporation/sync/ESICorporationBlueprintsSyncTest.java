...
public class ESICorporationBlueprintsSyncTest extends SyncTestBase {
  ...
  static {
    ...
    int locationFlagLen = GetCorporationsCorporationIdBlueprints200Ok.LocationFlagEnum.values().length;
    for (int i = 0; i < size; i++) {
      blueprintsTestData[i][0] = TestBase.getUniqueRandomLong();
      blueprintsTestData[i][1] = TestBase.getRandomLong();
      blueprintsTestData[i][2] = GetCorporationsCorporationIdBlueprints200Ok.LocationFlagEnum.values()[TestBase.getRandomInt(
          locationFlagLen)];
  ...
  public void setup() throws Exception {
    ...
    ESIEndpointSyncTracker.getOrCreateUnfinishedTracker(corpSyncAccount, ESISyncEndpoint.CORP_BLUEPRINTS, 1234L, null);
  ...
  private void setupOkMock() throws Exception {
    ...
    List<GetCorporationsCorporationIdBlueprints200Ok> blueprintList =
        Arrays.stream(blueprintsTestData)
              .map(x -> {
                GetCorporationsCorporationIdBlueprints200Ok nextBlueprint = new GetCorporationsCorporationIdBlueprints200Ok();
                nextBlueprint.setItemId((Long) x[0]);
                nextBlueprint.setLocationId((Long) x[1]);
                nextBlueprint.setLocationFlag((GetCorporationsCorporationIdBlueprints200Ok.LocationFlagEnum) x[2]);
    ...
    for (int i = 0; i < pages.length; i++) {
      ...
      ApiResponse<List<GetCorporationsCorporationIdBlueprints200Ok>> apir = new ApiResponse<>(200, headers,
                                                                                              blueprintList.subList(
                                                                                                  last,
                                                                                                  pages[i]));
      EasyMock.expect(mockEndpoint.getCorporationsCorporationIdBlueprintsWithHttpInfo(
  ...
  private void verifyDataUpdate() throws Exception {
    ...
    List<Blueprint> storedData = AbstractESIAccountSync.retrieveAll(testTime, (long contid, AttributeSelector at) ->
        Blueprint.accessQuery(corpSyncAccount, contid, 1000, false, at, AbstractESIAccountSync.ANY_SELECTOR,
  ...
  public void testSyncUpdate() throws Exception {
    ...
    // Perform the sync
    ESICorporationBlueprintsSync sync = new ESICorporationBlueprintsSync(corpSyncAccount);
    ...
    ESIEndpointSyncTracker syncTracker = ESIEndpointSyncTracker.getLatestFinishedTracker(corpSyncAccount,
                                                                                         ESISyncEndpoint.CORP_BLUEPRINTS);
    ...
    syncTracker = ESIEndpointSyncTracker.getUnfinishedTracker(corpSyncAccount, ESISyncEndpoint.CORP_BLUEPRINTS);
