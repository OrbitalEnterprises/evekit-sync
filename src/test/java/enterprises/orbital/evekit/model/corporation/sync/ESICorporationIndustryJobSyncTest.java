...
public class ESICorporationIndustryJobSyncTest extends SyncTestBase {
  ...
  static {
    ...
    int statusLen = GetCorporationsCorporationIdIndustryJobs200Ok.StatusEnum.values().length;
    for (int i = 0; i < size; i++) {
      ...
      jobTestData[i][14] = GetCorporationsCorporationIdIndustryJobs200Ok.StatusEnum.values()[TestBase.getRandomInt(
          statusLen)];
  ...
  public void setup() throws Exception {
    ...
    ESIEndpointSyncTracker.getOrCreateUnfinishedTracker(corpSyncAccount, ESISyncEndpoint.CORP_INDUSTRY, 1234L, null);
  ...
  private void setupOkMock() throws Exception {
    ...
    // Setup mock calls
    List<GetCorporationsCorporationIdIndustryJobs200Ok> jobList =
        Arrays.stream(jobTestData)
              .map(x -> {
                GetCorporationsCorporationIdIndustryJobs200Ok nextJob = new GetCorporationsCorporationIdIndustryJobs200Ok();
                nextJob.setJobId((Integer) x[0]);
                nextJob.setInstallerId((Integer) x[1]);
                nextJob.setFacilityId((Long) x[2]);
                nextJob.setLocationId((Long) x[3]);
                ...
                nextJob.setStatus((GetCorporationsCorporationIdIndustryJobs200Ok.StatusEnum) x[14]);
    ...
    for (int i = 0; i < pages.length; i++) {
      ...
      ApiResponse<List<GetCorporationsCorporationIdIndustryJobs200Ok>> apir = new ApiResponse<>(200, headers,
                                                                                          jobList.subList(
                                                                                              last,
                                                                                              pages[i]));
  ...
  private void verifyDataUpdate() throws Exception {
    ...
    List<IndustryJob> storedData = AbstractESIAccountSync.retrieveAll(testTime, (long contid, AttributeSelector at) ->
        IndustryJob.accessQuery(corpSyncAccount, contid, 1000, false, at, AbstractESIAccountSync.ANY_SELECTOR,
  ...
  public void testSyncUpdate() throws Exception {
    ...
    // Perform the sync
    ESICorporationIndustryJobSync sync = new ESICorporationIndustryJobSync(corpSyncAccount);
    ...
    syncTracker = ESIEndpointSyncTracker.getUnfinishedTracker(corpSyncAccount, ESISyncEndpoint.CORP_INDUSTRY);
