...
import enterprises.orbital.eve.esi.client.model.GetCorporationsCorporationIdBookmarksCoordinates;
import enterprises.orbital.eve.esi.client.model.GetCorporationsCorporationIdBookmarksFolders200Ok;
import enterprises.orbital.eve.esi.client.model.GetCorporationsCorporationIdBookmarksItem;
...
public class ESICorporationBookmarksSyncTest extends SyncTestBase {
  ...
  static {
    ...
    for (int i = 0; i < size; i++) {
      ...
      foldersTestData[i][2] = TestBase.getRandomInt();
    }
  ...
  public void setup() throws Exception {
    ...
    ESIEndpointSyncTracker.getOrCreateUnfinishedTracker(corpSyncAccount, ESISyncEndpoint.CORP_BOOKMARKS, 1234L, null);
  ...
  private void setupOkMock() throws Exception {
    ...
    List<GetCorporationsCorporationIdBookmarksFolders200Ok> foldersList =
        Arrays.stream(foldersTestData)
              .map(x -> {
                GetCorporationsCorporationIdBookmarksFolders200Ok nextFolder = new GetCorporationsCorporationIdBookmarksFolders200Ok();
                nextFolder.setFolderId((Integer) x[0]);
                nextFolder.setName((String) x[1]);
                nextFolder.setCreatorId((Integer) x[2]);
                return nextFolder;
              })
    ...
    for (int i = 0; i < foldersPages.length; i++) {
      ...
      ApiResponse<List<GetCorporationsCorporationIdBookmarksFolders200Ok>> apir = new ApiResponse<>(200, headers,
                                                                                                    foldersList.subList(
                                                                                                        last,
                                                                                                        foldersPages[i]));
    ...
    List<GetCorporationsCorporationIdBookmarks200Ok> bookmarksList =
        Arrays.stream(bookmarksTestData)
              .map(x -> {
                GetCorporationsCorporationIdBookmarks200Ok nextBookmark = new GetCorporationsCorporationIdBookmarks200Ok();
                ...
                GetCorporationsCorporationIdBookmarksItem item = new GetCorporationsCorporationIdBookmarksItem();
                item.setItemId((Long) x[6]);
                item.setTypeId((Integer) x[7]);
                nextBookmark.setItem(item);
                GetCorporationsCorporationIdBookmarksCoordinates coords = new GetCorporationsCorporationIdBookmarksCoordinates();
    ...
    for (int i = 0; i < bookmarksPages.length; i++) {
      ...
      ApiResponse<List<GetCorporationsCorporationIdBookmarks200Ok>> apir = new ApiResponse<>(200, headers,
  ...
  private void verifyDataUpdate() throws Exception {
    ...
    List<Bookmark> storedData = AbstractESIAccountSync.retrieveAll(testTime, (long contid, AttributeSelector at) ->
        Bookmark.accessQuery(corpSyncAccount, contid, 1000, false, at, AbstractESIAccountSync.ANY_SELECTOR,
  ...
  public void testSyncUpdate() throws Exception {
    ...
    // Perform the sync
    ESICorporationBookmarksSync sync = new ESICorporationBookmarksSync(corpSyncAccount);
    ...
    ESIEndpointSyncTracker syncTracker = ESIEndpointSyncTracker.getLatestFinishedTracker(corpSyncAccount,
                                                                                         ESISyncEndpoint.CORP_BOOKMARKS);
    ...
    syncTracker = ESIEndpointSyncTracker.getUnfinishedTracker(corpSyncAccount, ESISyncEndpoint.CORP_BOOKMARKS);
