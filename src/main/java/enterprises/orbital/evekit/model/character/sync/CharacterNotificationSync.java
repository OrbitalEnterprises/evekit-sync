...
public class CharacterNotificationSync extends AbstractCharacterSync {
  ...
  protected long processServerData(long time, SynchronizedEveAccount syncAccount, ICharacterAPI charRequest, Object data, List<CachedData> updates)
    ...
    for (INotification next : notes) {
      CharacterNotification note = new CharacterNotification(
          next.getNotificationID(), next.getTypeID(), next.getSenderID(), next.getSentDate().getTime(), next.isRead());
      updates.add(note);
    }
  ...
  public static SyncStatus syncNotifications(long time, SynchronizedEveAccount syncAccount, SynchronizerUtil syncUtil, ICharacterAPI charRequest) {
