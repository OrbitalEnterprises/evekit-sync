...
public class CorporationMemberMedalsSync extends AbstractCorporationSync {
  ...
  protected long processServerData(long time, SynchronizedEveAccount syncAccount, ICorporationAPI corpRequest, Object data, List<CachedData> updates)
    ...
    for (IMemberMedal next : medals) {
      CorporationMemberMedal newMedal = new CorporationMemberMedal(
          next.getMedalID(), next.getCharacterID(), next.getIssued().getTime(), next.getIssuerID(), next.getReason(), next.getStatus());
  ...
  public static SyncStatus syncCorporationMemberMedals(long time, SynchronizedEveAccount syncAccount, SynchronizerUtil syncUtil, ICorporationAPI corpRequest) {
